...
if [ ! -f "$RELEASE_DIR/MacDroidNotify-android-0.2.2.apk" ]; then
  echo "Expected release Android APK" >&2
  exit 1
fi

if [ ! -f "$RELEASE_DIR/MacDroidNotify-mac-0.2.2.zip" ]; then
...
if ! unzip -l "$RELEASE_DIR/MacDroidNotify-mac-0.2.2.zip" | grep "MacDroid Notify.app/Contents/Info.plist" >/dev/null; then
  echo "Expected Mac app bundle inside release zip" >&2
  exit 1
fi

if unzip -l "$RELEASE_DIR/MacDroidNotify-mac-0.2.2.zip" | grep '/\._\| \._' >/dev/null; then
